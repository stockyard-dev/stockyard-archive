...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/clips", s.list)
	s.mux.HandleFunc("POST /api/clips", s.create)
	s.mux.HandleFunc("GET /api/clips/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/clips/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-archive", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"clips": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Clip{} }
	wj(w, map[string]any{"clips": items, "count": len(items)})
}
